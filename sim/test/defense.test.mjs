...
/**
 * 提交级参考参数包（与磁盘当前值解耦，用于对比测试）。
 * @returns {import('../tunables_bundle.mjs').TunablesBundle} 参考参数包
 */
...
/**
 * 构造关闭防御的参数集。
 * @returns {import('../tunables_bundle.mjs').TunablesBundle} 关停防御的测试参数包
 */
 ...
 * @param {number} peerMinFloor 收集阶段下限
 * @param {number} peerMinRatio 收集阶段比例
 * @param {number} strictMinFloor 严格仲裁下限
 * @param {number} strictMinRatio 严格仲裁比例
 * @returns {import('../tunables_bundle.mjs').TunablesBundle} 调整后的参数包
 */
 ...
 * @param {import('../tunables_bundle.mjs').TunablesBundle} bundle 参数包
 * @param {number[]} [seeds] 随机种子列表
...
test('default tunables improve malicious suppression vs disabled on sybil_heavy', () => {
...
test('sybilContainmentRate is non-trivial on sybil_heavy', () => {
...
for (const [module, key, low, high] of GRADIENT_GUARDS)
	test(`param ${module}.${key} has a live simulation gradient`, () => {
...
test('federationFanoutTopK=1 loses fitness on eclipse_targeted (isolated federation resilience)', () => {
...
test('gutted fanout loses fitness on churn_storm (endogenous resilience)', () => {
...
test('federationFanoutTopK has interior optimum on eclipse_targeted', () => {
...
test('mailbox maxHop has interior optimum on churn_storm', () => {
...
test('over-strict archive quorum loses liveness on relay_mesh', () => {
...
test('default archive quorum avoids the over-strict liveness cliff on relay_mesh', () => {
...
test('key_thief and sleeper are containable on dedicated scenarios', () => {
	const tunables = loadDefaultTunables()
...
test('tiny_group archive quorum stays live at groupSize 3', () => {
	const tunables = loadDefaultTunables()
...
test('defense metrics are not saturated at 1.0 on balanced with defaults', () => {
	const tunables = loadDefaultTunables()
	const scenario = resolveScenarios('balanced')[0]
	const snap = runSimulation(scenario, 1, tunables)
