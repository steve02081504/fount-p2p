...
import { EXPLORE_MAX_PER_SOURCE as PEER_POOL_EXPLORE_MAX } from '../../transport/peer_pool.mjs'
import { MAX_SOURCE_SLOT_FRACTION, resolveRtcBudgetLimits } from '../../transport/rtc_connection_budget.mjs'
...
test('sim explore source quota tracks peer_pool', () => {
...
test('defaultConcurrency uses full logical CPU count', () => {
	const n = defaultConcurrency()
	let expected = 0
	const deno = globalThis.Deno
	if (deno && typeof deno.systemCpuInfo === 'function')
		try {
...
test('runSimulation serial snapshots are deterministic', () => {
...
test('parallel evaluate matches serial evaluate', async () => {
	const tunables = loadDefaultTunables()
	const scenario = resolveScenarios('balanced')[0]
	const seeds = [1, 2, 3]
...
test('parallel runSimulation jobs match direct runSimulation', async () => {
	const tunables = loadDefaultTunables()
...
test('batched evaluateMany matches per-candidate evaluate', async () => {
	const tunables = loadDefaultTunables()
	const scenario = resolveScenarios('balanced')[0]
	const seeds = [1, 2, 3]
	const attackPanel = [normalizeAttackGenome(undefined), randomAttackGenome(createRng(11))]
