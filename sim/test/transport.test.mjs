...
test('single source can still connect at least one peer', () => {
...
test('transport_siege scenario exposes transport metrics', () => {
...
test('transportMetrics tracks signaling diversity', () => {
	const state = createTransportState()
...
test('transportMetrics throttle uses simulation clock not wall clock', () => {
	const state = createTransportState()
...
test('transport_siege join throttle effective after sim decay', () => {
	const snap = runSimulation(resolveScenarios('transport_siege')[0], 3, loadDefaultTunables())
	assertEquals(typeof snap.joinThrottleEffectiveness, 'number')
