...
test('discoveryReach uses trusted anchors', () => {
...
test('eclipseFillExplore poisons explore set', () => {
	const state = createDiscoveryState()
...
test('recoverDiscoveryFromAnchors clears poison', () => {
	const state = createDiscoveryState()
...
test('trustedAnchors are per-observer not shared', () => {
	const state = createDiscoveryState()
...
test('eclipse scenario produces byAttackImpact reach data', () => {
...
test('reachCollapse attributed only to reach-type attacks', () => {
	const snap = runSimulation(resolveScenarios('spam_eclipse')[0], 5, loadDefaultTunables())
