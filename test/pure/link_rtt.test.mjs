 ...
 * @param {number} [initiatorPingDelayMs] 发起方 ping 的送达延迟；缺省为即时投递
...
function connectPipes(initiatorPingDelayMs) {
	...
	const optionsFor = (initiator, nodeHash, localIdentity) => ({
		...
		sendFrame: (action, frame) => {
			if (initiator && action === 'ping' && initiatorPingDelayMs !== undefined)
				setTimeout(() => deliverToResponder(frame), initiatorPingDelayMs)
...
test({
	 ...
	 */
	async fn() {
		const { initiatorPipe, responderPipe } = connectPipes(40)
		try {
			await Promise.all([initiatorPipe.startHandshake(), responderPipe.startHandshake()])
			await Promise.all([initiatorPipe.ready, responderPipe.ready])
