...
import { applyMdnsHostCandidatePolicy, filterMdnsIceCandidate } from '../../transport/rtc_mdns_filter.mjs'
...
test('applyMdnsHostCandidatePolicy rewrite-loopback rewrites .local host', () => {
...
test('applyMdnsHostCandidatePolicy drop discards .local host', () => {
	const sdp = 'candidate:1 1 udp 2130706431 abc123.local 54321 typ host generation 0'
	assertEquals(applyMdnsHostCandidatePolicy(sdp, 'drop'), null)
})

test('applyMdnsHostCandidatePolicy leaves srflx unchanged', () => {
...
test('filterMdnsIceCandidate rewrite-loopback rewrites mDNS host object', () => {
...
test('filterMdnsIceCandidate none passes through unchanged', () => {
	const raw = { candidate: 'candidate:0 1 udp 2130706431 x.local 9 typ host', sdpMid: '0', sdpMLineIndex: 0 }
