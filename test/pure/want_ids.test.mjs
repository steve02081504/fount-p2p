...
import {
	...
	wantIdsPeerKey,
} from '../../federation/want_ids.mjs'
import { assert, assertEquals } from '../helpers/assert.mjs'



test('resolveWantIdsLimits applies defaults and clamps bounds', () => {
...
test('wantIdsPeerKey joins group and peer with null separator', () => {
	const key = wantIdsPeerKey('group-a', 'peer-b')
	assertEquals(key, 'group-a\0peer-b')
})

test('takeIncomingWantIdsSlot enforces per-peer batch limit and backoff', () => {
	...
	assert.ok(isWantIdsInBackoff(peerKey))
})

test('takeOutgoingWantIdsSlot enforces per-group batch limit and backoff', () => {
	...
	assert.ok(isWantIdsInBackoff(groupId))
})

test('batchWantIds caps list to budget', () => {
...
test('recordWantIdsBackoff grows delay up to 120s cap', () => {
	...
	try {
		...
		assert.ok(isWantIdsInBackoff(key))
		now += 2_000
