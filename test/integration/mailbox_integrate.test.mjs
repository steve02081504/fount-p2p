...
async function withTempNodeDir(testFn) {
	const dir = await mkTestNodeDir('fount-mailbox-int-')
...
test('isDeliverableMailboxRecord rejects quarantine and missing fields', () => {
...
test('publishMailboxRecord stores trusted hop-0 record', async () => {
...
test('ingestMailboxPut rejects spoofed nodeHash when peerId is not bound', async () => {
	await withTempNodeDir(async () => {
...
test('ingestMailboxPut ignores forged low hop on wire (minimum relay hop 1)', async () => {
	await withTempNodeDir(async () => {
		await ingestMailboxPut({ replicaUsername: USER }, {
			nodeHash: FROM_NODE,
			record: {
...
test('ingestMailboxPut increments hop and stores relayed record', async () => {
	await withTempNodeDir(async () => {
		await ingestMailboxPut({ replicaUsername: USER }, {
			nodeHash: FROM_NODE,
			record: {
...
test('ingestMailboxPut drops at maxHop without storing', async () => {
	await withTempNodeDir(async () => {
		await ingestMailboxPut({ replicaUsername: USER }, {
			nodeHash: FROM_NODE,
			record: {
...
test('respondMailboxWant omits quarantine records', async () => {
	await withTempNodeDir(async () => {
...
test('ingestMailboxGive ignores quarantine wire records', async () => {
	await withTempNodeDir(async () => {
...
test('ingestMailboxGive dispatches trusted records to consumers', async () => {
	await withTempNodeDir(async () => {
