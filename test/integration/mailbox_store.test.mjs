...
test('normalizeMailboxHop clamps negatives and non-integers', () => {
...
test('negative hop normalized relay becomes hop 1 not trusted hop 0', () => {
...
test('storeMailboxRecord accepts quarantine tier at hop >= 2', async () => {
...
test('storeMailboxRecord concurrent writes keep both records', async () => {
	await withTempNodeDir(async () => {
...
test('relayHopAfterWireIngress enforces minimum hop 1 on wire', () => {
...
test('isMailboxRecordWithinSizeLimit rejects oversized envelope', () => {
...
test('readAll skips corrupt jsonl lines', async () => {
	await withTempNodeDir(async () => {
...
test('deleteMailboxRecords removes by envelope id', async () => {
	await withTempNodeDir(async () => {
